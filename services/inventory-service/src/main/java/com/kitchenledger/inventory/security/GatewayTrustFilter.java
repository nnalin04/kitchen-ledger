...
/**
 * Reads the trusted headers injected by the API Gateway after JWT verification.
 * Services downstream of the Gateway MUST NOT verify JWTs themselves.
 * Rejects any public-path request that is missing x-user-id or x-tenant-id as a
 * defense-in-depth measure against traffic that bypasses the gateway.
 */
@Component
